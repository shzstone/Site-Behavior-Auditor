...
function sba_install() {
    ...
    if ( empty( $cur ) ) update_option( 'sba_settings', $defaults );
        ...
        if ( isset( $cur['login_slug'] ) ) { unset( $cur['login_slug'] ); $updated = true; }
...
function sba_weekly_optimize() {
    ...
    $tables_to_optimize = [
        ...
        "{$wpdb->prefix}sba_threat_summary"
    ];
...
function sba_get_ip() {
    ...
    foreach ($header_keys as $key) {
        if (!empty($_SERVER[$key])) {
            ...
            foreach ($ip_list as $temp_ip) {
                $temp_ip = trim($temp_ip);
                // 核心：必须是有效的 IP，且跳过所有私有保留网段 (172.20.0.1 等)
    ...
    // 如果没找到公网 IP（内网环境测试），取 REMOTE_ADDR 兜底
    if (empty($ip)) {
...
function sba_is_internal_ip( $ip = null ) {
    $ip = $ip ?: sba_get_ip();
    if ( $ip === '172.20.0.1' || $ip === '127.0.0.1' || $ip === '::1' ) return true;
...
function sba_disable_user_endpoints( $endpoints ) {
    if ( ! is_user_logged_in() ) {
        ...
        unset( $endpoints['/wp/v2/posts'], $endpoints['/wp/v2/pages'] );
        unset( $endpoints['/wp/v2/comments'], $endpoints['/wp/v2/media'] );
        unset( $endpoints['/wp/v2/types'], $endpoints['/wp/v2/taxonomies'] );
...
function sba_rest_pre_dispatch( $result, $server, $request ) {
    if ( ! is_user_logged_in() && ! sba_is_ip_whitelisted() ) {
        ...
        $blocked_patterns = [ '/wp/v2/users', '/wp/v2/posts', '/wp/v2/pages', '/wp/v2/comments', '/wp/v2/media' ];
        foreach ( $blocked_patterns as $pattern ) {
            if ( strpos( $route, $pattern ) === 0 ) {
                return new WP_Error( 'rest_forbidden', __( '无权限访问此 API', SBA_TEXT_DOMAIN ), [ 'status' => 403 ] );
            }
        }
    }
    return $result;
...
function sba_execute_block( $reason ) {
    ...
    $wpdb->query( $wpdb->prepare(
        "INSERT INTO {$wpdb->prefix}sba_threat_summary (ip, total_blocks, last_block_time)
         VALUES (%s, 1, NOW())
         ON DUPLICATE KEY UPDATE total_blocks = total_blocks + 1, last_block_time = NOW()",
        $ip
    ) );
...
function sba_flush_pv_buffer( $date ) {
    $lock_key = 'sba_flush_pv_lock_process_' . $date;
...
function sba_stats_engine() {
    ...
    $ip = sba_get_ip();
    $sba_all_opts = get_option('sba_settings', []);

    $provided_key = isset($_GET['sba_key']) ? $_GET['sba_key'] : '';
    $emergency_key = isset($sba_all_opts['emergency_entrance_key']) ? $sba_all_opts['emergency_entrance_key'] : '';

    if ( !empty($provided_key) && !empty($emergency_key) && $provided_key === $emergency_key ) {
        $whitelist = isset($sba_all_opts['ip_whitelist']) ? (string)$sba_all_opts['ip_whitelist'] : '';
        if ( stripos( $whitelist, $ip ) === false ) {
            $sba_all_opts['ip_whitelist'] = trim( $whitelist ) . "\n" . $ip;
            update_option( 'sba_settings', $sba_all_opts );
            wp_cache_delete( 'sba_settings', 'options' );
        }
        header('Content-Type: text/html; charset=utf-8');
        die( sprintf(
            "<div style='padding:50px;text-align:center;font-family:sans-serif;background:#fff;max-width:700px;margin:50px auto;border:2px solid #28cd41;border-radius:20px;box-shadow:0 15px 35px rgba(0,0,0,0.1);'>
                <h1 style='color:#28cd41;font-size:60px;margin:0;'>%s</h1>
                <h2>%s</h2>
                <div style='text-align:left;background:#f8f9fa;padding:20px;border-radius:10px;margin:20px 0;'>
                    <p><b>%s</b> <code style='color:#007aff;font-size:16px;'>%s</code></p>
                    <p style='font-size:14px;color:#333;'>%s</p>
                </div>
                <p>%s</p>
                <p style='margin-top:20px;'><a href='%s' style='text-decoration:none;background:#28cd41;color:#fff;padding:10px 30px;border-radius:5px;'>%s</a></p>
            </div>",
            __('✔', SBA_TEXT_DOMAIN),
            __('物理级加白成功', SBA_TEXT_DOMAIN),
            __('识别到的真实公网 IP:', SBA_TEXT_DOMAIN),
            $ip,
            __('该地址已成功通过密钥验证并加入永久白名单。', SBA_TEXT_DOMAIN),
            __('请点击下方按钮返回，拦截已解除。', SBA_TEXT_DOMAIN),
            home_url(),
            __('返回首页', SBA_TEXT_DOMAIN)
        ));
    }
    ...
    if ( preg_match( '/\.sql$/', $uri ) ) sba_execute_block( __( '探测敏感文件: SQL', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.log$/', $uri ) ) sba_execute_block( __( '探测敏感文件: LOG', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.bak$/', $uri ) ) sba_execute_block( __( '探测敏感文件: BAK', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.env$/', $uri ) ) sba_execute_block( __( '探测敏感文件: ENV', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.git/', $uri ) ) sba_execute_block( __( '探测敏感文件: GIT', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.svn/', $uri ) ) sba_execute_block( __( '探测敏感文件: SVN', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/readme\.html$/', $uri ) ) sba_execute_block( __( '探测敏感文件: README', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/license\.txt$/', $uri ) ) sba_execute_block( __( '探测敏感文件: LICENSE', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/wp-config\.php/', $uri ) ) sba_execute_block( __( '探测敏感文件: CONFIG', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/phpinfo\.php/', $uri ) ) sba_execute_block( __( '探测敏感文件: PHPINFO', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.htaccess$/', $uri ) ) sba_execute_block( __( '探测敏感文件: HTACCESS', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.htpasswd$/', $uri ) ) sba_execute_block( __( '探测敏感文件: HTPASSWD', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/\.ini$/', $uri ) ) sba_execute_block( __( '探测敏感文件: INI', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/wp-mail\.php$/', $uri ) ) sba_execute_block( __( '探测敏感文件: MAIL', SBA_TEXT_DOMAIN ) );
    if ( preg_match( '/wp-links-opml\.php$/', $uri ) ) sba_execute_block( __( '探测敏感文件: OPML', SBA_TEXT_DOMAIN ) );

    if ( ! $is_bot && ! empty($ua) ) {
        if ( stripos( $ua, 'sqlmap' ) !== false ) sba_execute_block( __( '自动化扫描: sqlmap', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'nmap' ) !== false ) sba_execute_block( __( '自动化扫描: nmap', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'dirbuster' ) !== false ) sba_execute_block( __( '自动化扫描: dirbuster', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'nikto' ) !== false ) sba_execute_block( __( '自动化扫描: nikto', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'zgrab' ) !== false ) sba_execute_block( __( '自动化扫描: zgrab', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'python-requests' ) !== false ) sba_execute_block( __( '自动化扫描: python', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'go-http-client' ) !== false ) sba_execute_block( __( '自动化扫描: go-client', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'java/' ) !== false ) sba_execute_block( __( '自动化扫描: java', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'curl/' ) !== false ) sba_execute_block( __( '自动化扫描: curl', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'wget' ) !== false ) sba_execute_block( __( '自动化扫描: wget', SBA_TEXT_DOMAIN ) );
        if ( stripos( $ua, 'masscan' ) !== false ) sba_execute_block( __( '自动化扫描: masscan', SBA_TEXT_DOMAIN ) );
    }

    if ( strpos( $uri, '/.env' ) !== false ) sba_execute_block( __( '非法路径探测: .env', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/.git' ) !== false ) sba_execute_block( __( '非法路径探测: .git', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/.sql' ) !== false ) sba_execute_block( __( '非法路径探测: .sql', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/.ssh' ) !== false ) sba_execute_block( __( '非法路径探测: .ssh', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/wp-config.php.bak' ) !== false ) sba_execute_block( __( '非法路径探测: wp-config-bak', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/phpinfo.php' ) !== false ) sba_execute_block( __( '非法路径探测: phpinfo', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/config.php.swp' ) !== false ) sba_execute_block( __( '非法路径探测: swp', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/.vscode' ) !== false ) sba_execute_block( __( '非法路径探测: vscode', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/wp-links-opml.php' ) !== false ) sba_execute_block( __( '非法路径探测: opml', SBA_TEXT_DOMAIN ) );
    if ( strpos( $uri, '/wp-admin/install.php' ) !== false ) sba_execute_block( __( '非法路径探测: install', SBA_TEXT_DOMAIN ) );
    $custom_evil = array_filter( array_map( 'trim', explode( ',', sba_get_option( 'evil_paths', '' ) ) ) );
    foreach ( $custom_evil as $p ) { if ( ! empty( $p ) && strpos( $uri, $p ) !== false ) sba_execute_block( __( '非法路径探测: ', SBA_TEXT_DOMAIN ) . $p ); }

    if ( in_array( $_SERVER['REQUEST_METHOD'], array('POST', 'PUT') ) ) {
        $raw_p = file_get_contents( 'php://input' );
        $qs = $_SERVER['QUERY_STRING'] ?? '';
        if ( preg_match( '/(union\s+select|select\s+.*\s+from)/i', $qs ) || preg_match( '/(union\s+select|select\s+.*\s+from)/i', $raw_p ) ) sba_execute_block( __( '恶意载荷: SQL注入', SBA_TEXT_DOMAIN ) );
        if ( preg_match( '/(insert\s+into|update\s+set|delete\s+from)/i', $qs ) || preg_match( '/(insert\s+into|update\s+set|delete\s+from)/i', $raw_p ) ) sba_execute_block( __( '恶意载荷: 数据库改写', SBA_TEXT_DOMAIN ) );
        if ( preg_match( '/<script[^>]*>.*?<\/script>/is', $qs ) || preg_match( '/<script[^>]*>.*?<\/script>/is', $raw_p ) ) sba_execute_block( __( '恶意载荷: XSS脚本', SBA_TEXT_DOMAIN ) );
        if ( preg_match( '/javascript\s*:/i', $qs ) || preg_match( '/javascript\s*:/i', $raw_p ) ) sba_execute_block( __( '恶意载荷: JS脚本执行', SBA_TEXT_DOMAIN ) );
        if ( preg_match( '/(onabort|onerror|onload|onclick|onfocus|onmouseover|onmouseout|onchange|onsubmit)\s*=/i', $qs ) || preg_match( '/(onabort|onerror|onload|onclick|onfocus|onmouseover|onmouseout|onchange|onsubmit)\s*=/i', $raw_p ) ) sba_execute_block( __( '恶意载荷: 事件注入攻击', SBA_TEXT_DOMAIN ) );
    }
...
function sba_security_engine() {
    ...
    if ( strpos( $request_uri, 'wp-login.php' ) !== false || strpos( $script_name, 'wp-login.php' ) !== false || strpos( $request_uri, 'wp-signup.php' ) !== false ) {
        if ( $is_logged_in ) {
            if ( $action === 'logout' ) return;
            wp_redirect( admin_url() ); exit;
        }
        $allowed = array('rp', 'resetpass', 'logout', 'postpass', 'checkemail');
        if ( ! in_array( $action, $allowed ) ) { sba_execute_block( __( '物理封锁：入口禁用', SBA_TEXT_DOMAIN ) ); return; }
    }
    if ( is_admin() && ! $is_logged_in ) { sba_execute_block( __( '物理封锁：未经授权', SBA_TEXT_DOMAIN ) ); return; }

    $legit = array('register', 'lostpassword', 'retrievepassword', 'rp', 'resetpass', 'postpass', 'checkemail');
    ...
    if ( $limit > 0 && ! $is_logged_in && ! $is_bot && ! in_array( $ip, array('127.0.0.1', '::1') ) ) {
...
function sba_ajax_load_tracks() {
    ...
    if ( $rows ) {
        foreach ( $rows as $r ) {
            ...
            $display_ip = current_user_can('manage_options') ? esc_html( $r->ip ) : sba_mask_ip( $r->ip );
            $html .= "<tr>
                <td>" . date( 'H:i', strtotime( $r->last_visit ) ) . "</td>
                <td><code>{$display_ip}</code></td>
...
function sba_ajax_load_blocked_logs() {
    ...
    if ( $rows ) {
        foreach ( $rows as $b ) {
            $display_ip = current_user_can('manage_options') ? esc_html( $b->ip ) : sba_mask_ip( $b->ip );
            $html .= "<tr>
...
function sba_ajax_get_threat_ranking() {
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_send_json_error( __( '权限不足', SBA_TEXT_DOMAIN ) );
        return;
    }
    ...
    $total_count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}sba_threat_summary WHERE total_blocks > 0" );

    if ( empty( $total_count ) || $total_count == 0 ) {
        wp_send_json_success( array(
            'html' => '<tr><td colspan="5" style="text-align:center;">' . __( '暂无威胁数据', SBA_TEXT_DOMAIN ) . '</td></tr>',
            'has_next' => false,
            'total' => 0
        ) );
        return;
    ...
    $ranking = $wpdb->get_results( $wpdb->prepare(
        "SELECT ip, total_blocks, last_block_time
         FROM {$wpdb->prefix}sba_threat_summary
         WHERE total_blocks > 0
         ORDER BY CAST(total_blocks AS UNSIGNED) DESC
         LIMIT %d, %d",
        $offset,
        $per_page + 1
    ) );

    $has_next = count( $ranking ) > $per_page;
    if ( $has_next ) {
        array_pop( $ranking );
    }
    ...
    if ( ! empty( $ranking ) ) {
        ...
        foreach ( $ranking as $r ) {
            $html .= '<tr>
                ...
                <td style="color:#d63638;"><strong>' . number_format( intval( $r->total_blocks ) ) . '</strong></td>
                <td>' . date( 'Y-m-d H:i:s', strtotime( $r->last_block_time ) ) . '</td>
                <td><button class="button sba-ban-ip-btn" data-ip="' . esc_attr( $r->ip ) . '">' . __( '永久封禁', SBA_TEXT_DOMAIN ) . '</button></td>
            </tr>';
            $rank++;
        }
    } else {
...
function sba_ajax_ban_ip() {
    ...
    if ( ! filter_var( $ip, FILTER_VALIDATE_IP ) ) {
        wp_send_json_error( __( '无效的 IP 地址', SBA_TEXT_DOMAIN ) );
        return;
    }

    $current_whitelist = sba_get_option( 'ip_whitelist', '' );
    $whitelist_array = array_filter( array_map( 'trim', explode( "\n", $current_whitelist ) ) );

    if ( in_array( $ip, $whitelist_array ) ) {
        wp_send_json_error( __( '此 IP 在白名单中，无法封禁', SBA_TEXT_DOMAIN ) );
        return;
    }
    ...
    $ban_list_html = sba_generate_ban_list_html();

    wp_send_json_success( [
        'message' => sprintf( __( 'IP %s 已被永久封禁', SBA_TEXT_DOMAIN ), $ip ),
...
function sba_generate_ban_list_html() {
    ...
    if ( empty( $ban_list ) ) {
        return '<p>' . __( '暂无手动封禁的 IP。', SBA_TEXT_DOMAIN ) . '</p>';
    }

    $html = '<div class="sba-scroll-x">
        <table class="sba-table">
            <thead>
                <tr>
                    <th>' . __( 'IP 地址', SBA_TEXT_DOMAIN ) . '</th>
                    <th>' . __( '操作', SBA_TEXT_DOMAIN ) . '</th>
                </tr>
            </thead>
            <tbody>';
    ...
    foreach ( $ban_list as $ip ) {
        $html .= '<tr>
            <td><code>' . esc_html( $ip ) . '</code></td>
            <td><button class="button sba-unban-ip-btn" data-ip="' . esc_attr( $ip ) . '">' . __( '解除封禁', SBA_TEXT_DOMAIN ) . '</button></td>
        </tr>';
    }

    $html .= '</tbody> домаћинствима</div>';
...
function sba_ajax_sync_threat_ranking() {
    ...
    if ( $count > 0 ) {
        wp_send_json_success( [ 'message' => sprintf( __( '历史数据对账完成！共同步 %d 条记录', SBA_TEXT_DOMAIN ), $count ) ] );
    } else {
        wp_send_json_error( __( '对账失败，请检查数据库。', SBA_TEXT_DOMAIN ) );
    }
}
...
function sba_ajax_unban_ip() {
    ...
    if ( ( $key = array_search( $ip, $ban_list ) ) !== false ) {
        unset( $ban_list[$key] );
        update_option( 'sba_permanent_ban_list', array_values( $ban_list ) );

        $ban_list_html = sba_generate_ban_list_html();
        wp_send_json_success( [
            'message' => sprintf( __( 'IP %s 已解除封禁', SBA_TEXT_DOMAIN ), $ip ),
            'ban_list_html' => $ban_list_html
        ] );
    } else {
...
function sba_permanent_ban_check() {
    ...
    if ( in_array( $ip, $ban_list ) && ! sba_is_ip_whitelisted( $ip ) ) {
        status_header( 403 );
...
function sba_ios_login_shortcode() {
    ...
    if ( isset( $_GET['sba_message'] ) ) {
        switch ( $_GET['sba_message'] ) {
            case 'reset_sent':
                $message = __( '密码重置链接已发送到您的邮箱，请查收。', SBA_TEXT_DOMAIN );
                $msg_type = 'info';
                break;
            case 'reset_success':
                $message = __( '密码修改成功，请使用新密码登录。', SBA_TEXT_DOMAIN );
                $msg_type = 'success';
                break;
            case 'activation_success':
                $message = __( '账号激活成功，请登录。', SBA_TEXT_DOMAIN );
                $msg_type = 'success';
                break;
        }
    }
    ...
    if ( ! empty( $message ) ) {
        ...
        $message_html = '<div class="sba-ios-global-message" style="text-align:center;margin-bottom:15px;padding:12px;border-radius:12px;background:' . $color . '10;color:' . $color . ';font-size:14px;border:1px solid ' . $color . '20;">' . esc_html( $message ) . '</div>';
    }
...
function sba_threat_ranking_page() {
    ...
    <div class="wrap sba-wrap">
        ...
        <div class="sba-card">
            ...
            <div class="sba-scroll-x">
                <table class="sba-table">
                    <thead>
                        <tr>
                            <th><?php _e( '排名', SBA_TEXT_DOMAIN ); ?></th>
                            <th><?php _e( 'IP 地址', SBA_TEXT_DOMAIN ); ?></th>
                            <th><?php _e( '拦截次数', SBA_TEXT_DOMAIN ); ?></th>
                            <th><?php _e( '最后活动', SBA_TEXT_DOMAIN ); ?></th>
                            <th><?php _e( '操作', SBA_TEXT_DOMAIN ); ?></th>
                        </tr>
                    </thead>
                    <tbody id="threat-ranking-body">
                        <tr><td colspan="5"><?php _e( '加载中...', SBA_TEXT_DOMAIN ); ?></td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="sba-card" style="margin-top:20px; border-top: 3px solid #333;">
            ...
            <div id="sba-permanent-ban-list">
                <?php if ( empty( $ban_list ) ) : ?>
                    <p><?php _e( '暂无手动封禁的 IP。', SBA_TEXT_DOMAIN ); ?></p>
                <?php else : ?>
                    <div class="sba-scroll-x">
                        <table class="sba-table">
                            <thead>
                            ...
                            <tbody>
                                <?php foreach ( $ban_list as $ip ) : ?>
                                <tr>
                                    <td><code><?php echo esc_html( $ip ); ?></code></td>
                                    <td><button class="button sba-unban-ip-btn" data-ip="<?php echo esc_attr( $ip ); ?>"><?php _e( '解除封禁', SBA_TEXT_DOMAIN ); ?></button></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
    ...
    <script>
    var sba_i18n = {
        loading: '<?php _e( "加载中...", SBA_TEXT_DOMAIN ); ?>',
        loadFailed: '<?php _e( "加载失败", SBA_TEXT_DOMAIN ); ?>',
        unknownError: '<?php _e( "未知错误", SBA_TEXT_DOMAIN ); ?>',
        networkError: '<?php _e( "网络错误，请刷新页面重试", SBA_TEXT_DOMAIN ); ?>',
        banConfirm: '<?php _e( "确定要永久封禁该 IP 吗？", SBA_TEXT_DOMAIN ); ?>',
        unbanConfirm: '<?php _e( "确定要解除该 IP 的封禁吗？", SBA_TEXT_DOMAIN ); ?>',
        banning: '<?php _e( "封禁中...", SBA_TEXT_DOMAIN ); ?>',
        unbanning: '<?php _e( "解封中...", SBA_TEXT_DOMAIN ); ?>',
        banFailed: '<?php _e( "封禁请求失败", SBA_TEXT_DOMAIN ); ?>',
        unbanFailed: '<?php _e( "解封请求失败", SBA_TEXT_DOMAIN ); ?>',
        syncFailed: '<?php _e( "同步请求失败", SBA_TEXT_DOMAIN ); ?>',
        permanentBan: '<?php _e( "永久封禁", SBA_TEXT_DOMAIN ); ?>',
        removeBan: '<?php _e( "解除封禁", SBA_TEXT_DOMAIN ); ?>',
        syncing: '<?php _e( "同步中...", SBA_TEXT_DOMAIN ); ?>',
        syncHistory: '<?php _e( "🔄 同步历史数据", SBA_TEXT_DOMAIN ); ?>',
        prevPage: '<?php _e( "◀ 上一页", SBA_TEXT_DOMAIN ); ?>',
        nextPage: '<?php _e( "下一页 ▶", SBA_TEXT_DOMAIN ); ?>',
        page: '<?php _e( "第", SBA_TEXT_DOMAIN ); ?>',
        pageOf: '<?php _e( "页", SBA_TEXT_DOMAIN ); ?>'
    };
    ...
    jQuery(document).ready(function($) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        $('#threat-ranking-body').html('<tr><td colspan="5" style="text-align:center;color:#d63638;">' + sba_i18n.loadFailed + ': ' + (res.data || sba_i18n.unknownError) + '</td></tr>');
                    }
                    isLoading = false;
                },
                error: function() {
                    $('#threat-ranking-body').html('<tr><td colspan="5" style="text-align:center;color:#d63638;">' + sba_i18n.networkError + '</td></tr>');
                    isLoading = false;
                }
            });
        }
        ...
        $(document).on('click', '.sba-ban-ip-btn', function() {
            var ip = $(this).data('ip');
            if (!confirm(sba_i18n.banConfirm + ' ' + ip)) {
                return;
            }
            ...
            $.post(ajaxurl, {
                ...
                if (res.success) {
                    ...
                    if (res.data.ban_list_html) {
                        $('#sba-permanent-ban-list').html(res.data.ban_list_html);
                    }
                    loadThreatRanking(currentPage);
                } else {
                    alert(res.data);
                    btn.prop('disabled', false).text(originalText);
                }
            }, 'json').fail(function() {
                alert(sba_i18n.banFailed);
                btn.prop('disabled', false).text(originalText);
            });
        });
        ...
        $(document).on('click', '.sba-unban-ip-btn', function() {
            var ip = $(this).data('ip');
            if (!confirm(sba_i18n.unbanConfirm + ' ' + ip)) {
                return;
            }
            ...
            $.post(ajaxurl, {
                ...
                alert(sba_i18n.unbanFailed);
                btn.prop('disabled', false).text(originalText);
            });
        });

        $('#sba-prev-page').off('click').on('click', function() {
            if (currentPage > 1 && !isLoading) {
                loadThreatRanking(currentPage - 1);
            }
        });

        $('#sba-next-page').off('click').on('click', function() {
            if (!isLoading) {
                loadThreatRanking(currentPage + 1);
            }
        });
        ...
        $('#sba-sync-btn').off('click').on('click', function() {
            ...
            $.post(ajaxurl, {
                action: 'sba_sync_threat_ranking'
            }, function(res) {
                if (res.success) {
                    alert(res.data.message);
                } else {
                    alert(res.data || sba_i18n.syncFailed);
                }
                btn.prop('disabled', false).text(sba_i18n.syncHistory);
                loadThreatRanking(1);
            }, 'json').fail(function() {
                alert(sba_i18n.syncFailed);
                btn.prop('disabled', false).text(sba_i18n.syncHistory);
            });
        });
    ...
    <div class="wrap sba-wrap">
        ...
        <form method="post" action="options.php">
            ...
            <div class="sba-grid">
                ...
                <tr>
                    <th><?php _e( '应急密钥', SBA_TEXT_DOMAIN ); ?></th>
                    <td>
                        <input type="text" name="sba_settings[emergency_entrance_key]" value="<?php echo esc_attr( sba_get_option('emergency_entrance_key') ); ?>" class="regular-text" placeholder="?sba_key=xxx" />
                        <br><small class="description"><?php _e( '访问 <code>域名/?sba_key=密钥</code> 可自动加白名单。', SBA_TEXT_DOMAIN ); ?></small>
                    </td>
                </tr>
...
function sba_smtp_page() {
    ...
    <div class="wrap"><h1><?php _e( 'SMTP 邮件设置', SBA_TEXT_DOMAIN ); ?></h1>
        ...
        <form method="post"><?php wp_nonce_field( 'sba_smtp_save' ); ?>
            <table class="form-table">
                <tr><th><label for="test_to"><?php _e( '接收测试邮箱', SBA_TEXT_DOMAIN ); ?></label></th><td><input type="email" id="test_to" name="test_to" class="regular-text" placeholder="your@email.com"></td></tr>
            </table>
