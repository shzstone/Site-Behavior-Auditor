...
function sba_cleanup_old_data() {
    ...
    $wpdb->query("DELETE FROM {$wpdb->prefix}sba_blocked_log WHERE block_time < DATE_SUB(NOW(), INTERVAL 30 DAY)");
...
function sba_audit_dashboard() {
    ...
    <div class="wrap sba-wrap">
        ...
        <div class="sba-grid">
            ...
            <div class="sba-card"><h3><?php _e( '📊 30天审计详表', SBA_TEXT_DOMAIN ); ?></h3><div class="sba-scroll-x" style="height:250px;"><table class="sba-table sba-audit-table"><thead><tr><th><?php _e( '日期', SBA_TEXT_DOMAIN ); ?></th><th><?php _e( 'UV (人)', SBA_TEXT_DOMAIN ); ?></th><th><?php _e( 'PV (次)', SBA_TEXT_DOMAIN ); ?></th><th><?php _e( '拦截 (次)', SBA_TEXT_DOMAIN ); ?></th><th><?php _e( '深度', SBA_TEXT_DOMAIN ); ?></th></tr></thead><tbody><?php for ( $i = 0; $i < 30; $i++ ): $d = date( 'Y-m-d', $end_ts - ( $i * 86400 ) ); $u = sba_get_uv( $d ); $p = sba_get_pv( $d ); $b = sba_get_blocked( $d ); ?><tr><td><b><?php echo $d; ?></b></td><td><?php echo $u; ?></td><td><?php echo $p; ?></td><td style="color:#d63638;"><?php echo $b; ?></td><td><code><?php echo ($u > 0) ? round( $p / $u, 1 ) : 0; ?></code></td></tr><?php endfor; ?></tbody></table></div></div>
        </div>
